...
class BartMoEModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.sentEncoder = BartEncoderSent(config,self.shared)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            sent_encoder_outputs = self.sentEncoder(
                encoder_outputs[0]
            )
            #6*99*768
...
class BartEncoderSent(nn.Module):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer
    is a :class:`EncoderLayer`.
    ...
    def forward(
        ...
        inputs_embeds = self.embed_tokens(embed) * self.embed_scale

        embed_pos = self.embed_positions(embed)
        x = inputs_embeds + embed_pos
