...
class BartMoEModel(PretrainedBartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #encoder_output[0], (batch_size*3)*seqlen*embed
        ...
        encoder_sent_outputs = self.sent_encoder(
            input_ids=encoder_outputs[0][:, 0],
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        )
        encoder_target_outputs = self.encoder(
        ...
        x = encoder_outputs[0]
        # sent_out = self.sent_encoder(x, torch.zeros(x.size()[:-1], dtype=torch.long).cuda(), torch.ones(x.size()[:-1], dtype=torch.long).cuda())
        # # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        decoder_outputs = self.decoder(
            decoder_input_ids,
            encoder_outputs[0]+encoder_sent_outputs[0],
            attention_mask,
        ...
        if not return_dict:
            return decoder_outputs + encoder_outputs+encoder_sent_outputs+encoder_target_outputs
...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        #self.embed_tokens Embedding 50265*768,scale:1.0
        #batch_sizr*len*embed
...
class BartSentEncoder(nn.Module):
    ...
    def forward(
        ...
        return BaseModelOutput(last_hidden_state=x, hidden_states=encoder_states, attentions=all_attentions)
class BartMoEForConditionalGeneration(PretrainedBartModel):
        ...
        **model_kwargs
        ...
        if self.config.is_encoder_decoder:
            ...
            if self.config.mixture_embedding:
                mixture_ids = torch.arange(num_beams, dtype=torch.long, device=input_ids.device).view(
