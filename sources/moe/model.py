...
class BartMoEForConditionalGeneration(PretrainedBartModel):
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past, attention_mask, use_cache, encoder_outputs,
                                      **kwargs):
        return {
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            if self.config.mixture_embedding:
                ...
                encoder_outputs = encoder(input_ids, mixture_ids=mixture_ids, attention_mask=attention_mask,
                                          return_dict=True)
        ...
        if self.config.is_encoder_decoder:
            ...
            input_ids = torch.full(
                (effective_batch_size * num_beams, 1), decoder_start_token_id, dtype=torch.long,
                device=next(self.parameters()).device, )
            cur_len = 1
            assert (
            ...
            expanded_batch_idxs = (
                torch.arange(batch_size).view(-1, 1).repeat(1, num_beams * effective_batch_mult).view(-1).to(
                    input_ids.device))
