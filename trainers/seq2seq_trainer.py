...
class Seq2SeqTrainer(Trainer):
    ...
    def _compute_loss(self, logits,cos_labels, cos_target,labels):
        if self.args.label_smoothing == 0:
            ...
            # with torch.no_grad:
            #     self.model.eval()
            #     cos_target = self.model(labels)[1]
            #     cos_target = cos_target[:,0]
            #     cos_labels = cos_labels[:,0]
            #     self.model.train()
            cos_loss = 1-(cos(cos_labels[:,0],cos_target[:,0]).mean())
            loss = loss_fct(logits.view(-1, logits.shape[-1]), labels.view(-1))+cos_loss
        else:
    ...
    def _compute_cos_loss(self, logits, labels):
        if self.args.label_smoothing == 0:
            loss_fct = torch.nn.CrossEntropyLoss(ignore_index=self.config.pad_token_id)
            assert logits.shape[-1] == self.vocab_size
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            x = outputs[0]
            loss = self._compute_loss(logits=outputs[1],cos_labels=outputs[3],cos_target=outputs[4], labels=labels_out)
            loss = loss.mean().item()
